...
FROM inbound_traffic inbound
         JOIN outbound_traffic outbound
              ON inbound.target_port = outbound.target_port AND
                 ...
                 cast(concat(inbound.date, ' ', inbound.hour + ':00:00') as datetime) >= now() - INTERVAL %d HOUR AND
                 cast(concat(outbound.date, ' ', outbound.hour + ':00:00') as datetime) >= now() - INTERVAL %d HOUR
